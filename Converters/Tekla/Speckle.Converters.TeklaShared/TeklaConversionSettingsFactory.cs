...
) : ITeklaConversionSettingsFactory
{
  ...
  public TeklaConversionSettings Create(
    Model document,
    bool sendRebarsAsSolid,
    ReceiveMode receiveMode = ReceiveMode.Native
  ) => new(document, sendRebarsAsSolid, unitsConverter.ConvertOrThrow(Distance.CurrentUnitType), receiveMode);
}
