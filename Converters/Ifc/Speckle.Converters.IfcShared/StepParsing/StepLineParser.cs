...
#else
  // Scalar fallback for net48 (no System.Runtime.Intrinsics there at all) - produces byte-for-byte
  // identical offsets to the AVX2 path above, just without the SIMD speedup. Takes a pointer to the
  // same 32-byte chunk StepDocument's loop already indexes into, so the calling loop's structure
  // (iterate NumVectors chunks of 32 bytes, advance index by 32 each time) is unchanged between TFMs.
  [MethodImpl(MethodImplOptions.AggressiveInlining)]
  public static unsafe void ComputeOffsets(byte* chunk, ref int index, List<int> offsets)
  {
    for (int i = 0; i < 32; i++)
    {
      if (chunk[i] == (byte)'\n')
