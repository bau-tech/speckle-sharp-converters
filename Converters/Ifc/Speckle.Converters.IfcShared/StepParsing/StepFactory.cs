...
public static unsafe class StepFactory
{
  ...
  public static StepValue Create(ref StepToken token, byte* end)
  {
    switch (token.Type)
    {
      ...
      default:
        throw new IfcStepParsingException($"Cannot convert token type {token.Type} to a StepValue");
    }
  }
  ...
  public static StepList CreateAggregate(ref StepToken token, byte* end)
  {
    ...
    if (token.Type != StepTokenType.BeginGroup)
      throw new IfcStepParsingException("Expected '('");
    ...
    throw new IfcStepParsingException("Unexpected end of input");
  }
}
